...
export async function getGroupTasks(groupId) {
    const res = await apiFetch(`/tasks/group/${groupId}`);
    return res.json();
}
...
export async function updateGroupTask(groupId, taskId, task) {
    const res = await apiFetch(`/tasks/group/${groupId}/${taskId}`, {
        method: "PUT",
        body: JSON.stringify(task),
    });
    return res.json();
...
export async function toggleGroupTask(groupId, taskId) {
    const res = await apiFetch(`/tasks/group/${groupId}/${taskId}/toggle`, {
        method: "PATCH"
    });
    return res.json();
}
export async function deleteGroupTask(groupId, taskId) {
    await apiFetch(`/tasks/group/${groupId}/${taskId}`, {
        method: "DELETE"
    });
}